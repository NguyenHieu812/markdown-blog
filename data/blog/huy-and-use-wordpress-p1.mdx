...
- Mở phần mềm (XAMPP Control Panel).
- Start các dịch vụ: <br /> - Apache (Web Server). <br /> - MySQL (Database). <br />
  > Thường Xampp sẽ cần cấu hình port 80, nếu bạn đã cài đặt một số phần mềm khác như Skype, Teamviewer,.. thì có thể sẽ phải cần tìm cách thay đổi port hoặc tắt phần mềm đó đi. <br />

Bước 2: <br />

- Truy cập phpMyAdmin: <br />
- Mở trình duyệt, nhập: http://localhost/phpmyadmin.
  <ul>
    <li>Nhấn New.</li>
    <li>Nhập tên cơ sở dữ liệu (ví dụ: wordpress).</li>
    <li>Chọn utf8_general_ci ở mục Collation.</li>
    <li>Nhấn Create.</li>
  </ul>

Bước 3: <br />
Tải WordPress:
Truy cập: https://wordpress.org và tải phiên bản mới nhất.

Giải nén WordPress:
Giải nén file tải về.
Sao chép thư mục WordPress vào thư mục của local server:
XAMPP: C:\xampp\htdocs\
MAMP: /Applications/MAMP/htdocs/
WAMP: C:\wamp\www\
Đổi tên thư mục WordPress nếu cần (ví dụ: mywebsite).

Bước 4: <br />

Truy cập WordPress qua trình duyệt:

Mở trình duyệt, nhập: http://localhost/mywebsite (thay mywebsite bằng tên thư mục bạn vừa tạo).
Bắt đầu quá trình cài đặt:

Chọn ngôn ngữ.
Nhập thông tin cơ sở dữ liệu:
Database Name: Tên cơ sở dữ liệu bạn đã tạo (ví dụ: wordpress).
Username: root (mặc định trong local server).
Password: Để trống (nếu dùng XAMPP) hoặc nhập mật khẩu mặc định của phần mềm.
Database Host: localhost.
Table Prefix: wp\_ (mặc định, hoặc thay đổi nếu muốn).
Nhấn Submit (Gửi).
Hoàn tất cài đặt:
...
Truy cập: http://localhost/mywebsite/wp-admin.
Đăng nhập bằng tài khoản quản trị vừa tạo.

Bước 5: <br />
Cài Themes và Plugins từ dashboard của WordPress.
Điều chỉnh cấu hình trong Settings:
General: Tên website, khẩu hiệu.
Permalinks: Đường dẫn URL.
Phát triển hoặc thử nghiệm tính năng tùy thích.
